...
from .text_utils import estimate_tokens, truncate_sentences
...
class _Buffer:
    ...
    def add_content(self, blk: ParsedBlock) -> None:
        ...
        self.parts.append(
            _Part(
                text=blk.text,
                start=blk.offset[0] if blk.offset else self.start_offset,
                end=blk.offset[1] if len(blk.offset) > 1 else self.start_offset,
                page_no=blk.page_no,
                bbox=blk.bbox,
                label=blk.label,
            )
    ...
    def flush(overlap: bool = False) -> None:
        ...
        if not buf.is_empty():
            ...
            if c:
                chunks.append(c)
                index += 1
        carry = buf.take_tail(_OVERLAP_TOKENS) if overlap else []
    ...
    for blk in blocks:
        ...
        if blk.text.strip():
            ...
            if estimate_tokens(blk.text) > _TOKEN_CAP:
                # 逐级下切至句子边界（无标点长串由 truncate_sentences 兜底）
                for sent in truncate_sentences(blk.text, _TOKEN_CAP):
                    ...
                    buf.add_content(
                        ParsedBlock(
                            ...
                            page_no=blk.page_no,
                            bbox=blk.bbox,
                            offset=[blk.offset[0], blk.offset[0] + len(sent)],
                        )
                    )
                flush(overlap=True)
            else:
