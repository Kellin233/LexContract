...
from ..utils.conversation_recorder import recorder as _conv_recorder
...
# 输入上下文窗口上限（token，统一口径见 src/utils/tokens.py）。默认 128K。
# 超过此阈值才触发"丢旧轮次"截断；可作为模型窗口的保险丝，也可按后端在
# configs/default.yaml 的 model.context_window_tokens 覆盖（VLLMPolicy.max_context_tokens）。
...
class VLLMPolicy:
    ...
    def _truncate_messages(self, messages: list, max_tokens: int | None = None) -> list:
        """主动截断：保留 system + 最近交互，逐步丢弃旧轮次（token 口径）。

        阈值 max_tokens（默认 self.max_context_tokens）才会触发；普通链路远够不到。
        截断是"丢弃旧轮次"而非截断内容，避免在消息中间切断导致语义破碎。
        """
        max_tokens = int(max_tokens or self.max_context_tokens)
        ...
        print(f"[TRUNCATE] Triggered: {before_tokens} tokens > {max_tokens} threshold. n_msgs={len(messages)}")
        ...
        while len(kept) > 3:
            ...
            if isinstance(removed, dict) and removed.get("role") == "assistant" and removed.get("tool_calls"):
                while kept and isinstance(kept[0], dict) and kept[0].get("role") == "tool":
                    kept.pop(0)
            after_tokens = estimate_messages_tokens(system_msgs + kept)
            if after_tokens <= max_tokens:
                print(f"[TRUNCATE] Reduced to {after_tokens} tokens, kept {len(kept)} non-system msgs")
                return system_msgs + kept

        # 极端情况：即使只保留 system + 最后 3 条也超阈值
        # 对最后一条（最新的交互）做内容级截断兜底
        after_tokens = estimate_messages_tokens(system_msgs + kept)
        if after_tokens > max_tokens and kept:
            # 截断最后一条 message 的 content（通常是超长的 tool result）
            last_msg = kept[-1]
            base = {k: v for k, v in last_msg.items() if k != "content"}
            base_tokens = estimate_messages_tokens([base])
            # 下限随预算按比例收缩（预算很小时不至于超过上限太多），至少保留一点最新内容
            floor = max(1, max_tokens // 8)
            keep_tokens = max(max_tokens - base_tokens - 32, floor)
            new_content = self._trim_to_tokens(str(last_msg.get("content", "")), keep_tokens)
            last_msg["content"] = new_content + "\n[CONTENT_TRUNCATED]"
            final_tokens = estimate_messages_tokens(system_msgs + kept)
            print(f"[TRUNCATE] Content-truncated last msg to ~{keep_tokens} tokens. Final: {final_tokens}")
            return system_msgs + kept

        return system_msgs + kept

    @staticmethod
    def _trim_to_tokens(text: str, keep_tokens: int) -> str:
        """按估算 token 预算取文本前缀（兜底内容截断用，极少触发）。"""
        if keep_tokens <= 0 or not text:
            return ""
        low, high = 0, len(text)
        while low < high:
            mid = (low + high + 1) // 2
            if estimate_tokens(text[:mid]) <= keep_tokens:
                low = mid
            else:
                high = mid - 1
        return text[:low]
    ...
    def __call__(self, messages: list) -> OpenAICompatibleDict:
        ...
        # 2. 主动截断（token 口径保险丝，远超普通链路才会触发）
        sanitized = self._truncate_messages(sanitized, max_tokens=self.max_context_tokens)
        ...
        try:
            ...
            _conv_recorder.record_llm_call(
                messages,
                result,
                usage=_usage,
            ...
            if "maximum context length" in err_lower or "context length" in err_lower:
                ...
                raise RuntimeError(
                    f"[CONTEXT_LENGTH_EXCEEDED] n_msgs={n_msgs}, est_tokens={est_tokens}: {err_str}"
                ) from e
